class ComponentController < ApplicationController
  ...
  def component_states
    ...
    program_steps = Program.find(step.program_id).select_program_steps(params[:id])
    program_steps.each  do |step|
      set_component_states(step, component_hash)
    end
    render json: component_hash.values
  end

  def set_component_states(step, component_hash)
    step.component_states.each do |component_state|
