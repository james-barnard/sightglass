...
class GraphicWindow extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (this.props.selectedComponent !== nextProps.selectedComponent)
      nextProps.selectedComponent && console.log(`this.props.selectedcomponent is ${nextProps.selectedComponent}`)
  };
  ...
  whichPic = (component) => {
    ...
    if (component.state === "closed") {
      return (<Image src={ nc_closed } key={component.id} id={'valve' + component.id} className={className} /> )
    } else if (component.state === "open") {
      return (<Image src={ nc_open } key={component.id} id={'valve' + component.id} className={className} /> )
    } else {
      return null;
    }
  }
